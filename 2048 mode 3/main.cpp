...
static int count_empty(board_t x)
{
    ...
    //  1 if the original nibble was non-zero
    //  0 if the original nibble was zero
    // Next sum them all
    x += x >> 32;
    x += x >> 16;
    x += x >>  8;
...
int find_best_move(board_t board) {
	...
	setcolor(BLACK);
    setfontbkcolor(EGERGB(187, 172, 158));
    setfont(25, 0, "Tahoma");
	rectprintf(10, 145, 430, 40, "Heur: %.0f\tActual: %.0f", score_heur_board(board), score_board(board));//output2
    ...
    for(move=0; move<4; move++) {
        ...
        if(res){
        	if (!best) { best =res; bestmove = move; continue; }
			if (res < best) { best = res; bestmove = move; }
		}
...
static board_t insert_tile_rand(board_t board, board_t tile) {
    ...
    board_t tmp = board;
    while (true) {
        while ((tmp & 0xf) != 0) { //10-1. tile move left by 1 if the box is not blank.
            tmp >>= 4;
            tile <<= 4;
        }
        if (index == 0) break;
        --index;
        tmp >>= 4;
        tile <<= 4; //10-2. tile move left by 1 if the index is not 0.
    }
    return board | tile;
}
...
void play_game() {
    ...
    while(1) {
        ...
        setfontbkcolor(EGERGB(187, 172, 158));
        setfont(25, 0, "Tahoma");
		rectprintf(160, 10, 100, 100, "Move #\n\n%d ", ++moveno);
        rectprintf(290, 10, 100,100, "Current\nScore\n%.0f", score_board(board) - scorepenalty);
		//printf("\nMove #%d, current score=%.0f\n", ++moveno, score_board(board) - scorepenalty); //output 1
        ...
        print_board(board);//output 3

        setcolor(BLACK);
    	setfont(20, 0, "Tahoma");
    	rectprintf(75, 625, 300, 45, "It's your turn!(enter {w,s,a,d})\n");// output for warning
		//printf("It's your turn!(enter {w,s,a,d})\n");// output for warning

        /*user input and execute*/
        ...
        switch(m){ //??????wsad???0123
            case 119: //w
            {
				...
				setfontbkcolor(EGERGB(187, 172, 158));
    			setfont(25, 0, "Tahoma");
    			outtextrect(301, 625, 450, 45, "Move     Up");
				break; //“
			}
			case 115://s
            {
				...
				setfontbkcolor(EGERGB(187, 172, 158));
    			setfont(25, 0, "Tahoma");
    			outtextrect(301, 625, 450, 45, "Move  down");
				break;
			}
			 case  97://a
			{
				...
				setfontbkcolor(EGERGB(187, 172, 158));
    			setfont(25, 0, "Tahoma");
    			outtextrect(301, 625, 450, 45, "Move    Left");
				break;  //“
			}
			case 100://d
			{
				...
				setfontbkcolor(EGERGB(187, 172, 158));
    			setfont(25, 0, "Tahoma");
    			outtextrect(301, 625, 450, 45, "Move  Right");
				break;
			}
			
           
            
            default : printf("Sorry, illegal input!\n");continue;
        }
        ...
        board_t ti = draw_tile();
        if (ti == 2) scorepenalty += 4;
        board = insert_tile_rand(newboard, ti);

        print_board(board);
    }

    int temp = score_board(board) - scorepenalty;
     setfontbkcolor(EGERGB(187, 172, 158));
    setfont(25, 0, "NEW TIMES ROMAN");
    rectprintf(490, 10, 100,100, "Game over. Your score is %.0f. The highest rank you achieved was %d.\n", score_board(board) - scorepenalty, get_max_rank(board));
    //printf("\nGame over. Your score is %.0f. The highest rank you achieved was %d.\n", score_board(board) - scorepenalty, get_max_rank(board));

}

int main() {
	initgraph(450, 700);//initialize a graph of size 450*700
	PIMAGE img;
	img = newimage(); 
	//set the background color for the game interface  
    setbkcolor(EGERGB(238, 228, 217));
	//create a solid rectangle with specified color as background color of board 
    setfillcolor(EGERGB(187, 172, 158));
    bar(0, 200, 480, 600);//Draw a solid rectangle on from (0,200) to (480,600)
    //get the icon image and put it on specified location
    getimage(img, "2048\\icon.jpg");   
	putimage( 10, 10,120,120,img,0,0,395,395);
	//create a solid rectangle with specified color for number of move table
	setfillcolor(EGERGB(187, 172, 158));
    bar(160, 10, 260, 120);
    //create a solid rectangle with specified color for current score table
    setfillcolor(EGERGB(187, 172, 158));
    bar(290, 10, 440, 120);
    //create a solid rectangle with specified color for heur and actul score table
    setfillcolor(EGERGB(187, 172, 158));
    bar(10, 140, 440, 180);
